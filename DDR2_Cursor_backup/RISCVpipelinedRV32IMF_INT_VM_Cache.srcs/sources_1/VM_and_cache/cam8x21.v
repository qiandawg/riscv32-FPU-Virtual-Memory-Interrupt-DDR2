...
************************************************/
module cam8x21 (clk,pattern,wraddress,wren,flush,maddress,mfound);          // cam
    input        clk;            // clock
    ...
    // write / flush
    integer k;
    // On flush=1, invalidate all lines by clearing only the valid bit [20].
    // Otherwise, normal CAM write: store {valid=1, pattern}.
    always @ (posedge clk) begin
        if (flush) begin
            for (k = 0; k < 8; k = k + 1)
                ram[k][20] <= 1'b0;         // invalidate line k
        end else if (wren) begin
            ram[wraddress] <= {1'b1, pattern};  // valid, pattern
        end
    end
