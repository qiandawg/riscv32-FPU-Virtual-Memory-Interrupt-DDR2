...
create_clock -period 100.000 -name jtag_tck -waveform {0.000 50.000} -add [get_ports {JB[4]}]
...
# Bank 34 shares VCCO with DDR2 (1.8 V), so SW[8]/SW[9] must be LVCMOS18.
# See Digilent Nexys A7 master XDC: T8/U8 are the only switches in bank 34.
...
# =============================================================================
# DDR2 / MIG cross-clock-domain constraints
# -----------------------------------------------------------------------------
# The CPU runs on clk_wiz_0/CLKOUT0 (50 MHz), and the MIG runs on its own
# ui_clk generated internally from the 200 MHz reference produced by pll_mem.
# These two trees are logically asynchronous: every signal that crosses them
# goes through the 2-FF ff_sync / flag_sync synchronizers inside mem_example.
# Without the groups below Vivado tries (and fails) to close static timing on
# the 64-bit data bus + handshake signals, producing WNS ~ -9 ns and
# TNS ~ -10 us, which is what made route_design appear to "hang".
#
# NOTE on hierarchy (keep these two lines in sync with the RTL):
#   - CPU clock pin  : clk_wiz_0/inst/mmcm_adv_inst/CLKOUT0
#   - pll_mem pin    : pll_mem/inst/mmcm_adv_inst/CLKOUT0    (200 MHz ref to MIG)
#   - MIG MMCM pin   : cpu/u_phys_mem/u_memex/mig1/u_mig_mig/u_ddr2_infrastructure/gen_mmcm.mmcm_i/CLKFBOUT
# =============================================================================

set_clock_groups -name cpu_to_mig_async -asynchronous \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins clk_wiz_0/inst/mmcm_adv_inst/CLKOUT0]] \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins cpu/u_phys_mem/u_memex/mig1/u_mig_mig/u_ddr2_infrastructure/gen_mmcm.mmcm_i/CLKFBOUT]]

set_clock_groups -name pllmem_to_mig_async -asynchronous \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins pll_mem/inst/mmcm_adv_inst/CLKOUT0]] \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins cpu/u_phys_mem/u_memex/mig1/u_mig_mig/u_ddr2_infrastructure/gen_mmcm.mmcm_i/CLKFBOUT]]

set_clock_groups -name cpu_to_pllmem_async -asynchronous \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins clk_wiz_0/inst/mmcm_adv_inst/CLKOUT0]] \
    -group [get_clocks -include_generated_clocks -of_objects [get_pins pll_mem/inst/mmcm_adv_inst/CLKOUT0]]
